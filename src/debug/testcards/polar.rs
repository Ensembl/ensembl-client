...
use shape::{
    ...
    fixunder_rectangle, fixunder_texture,
...
fn one_offs(c: &mut Component, p: &Palette) {
    c.add_shape(fix_rectangle(&area(cedge(TOPLEFT,cpixel(0,2)),
                                ...
                                &p.white));
    c.add_shape(fix_rectangle(&area(cedge(TOPLEFT,cpixel(36,2)),
                                    cedge(TOPLEFT,cpixel(37,17))),
                                &p.grey));
    let tx = text_texture("bp",
                          &p.lato_12,&Colour(199,208,213),&Colour(255,255,255));
    c.add_shape(fix_texture(tx,&cedge(TOPLEFT,cpixel(4,2)),&cpixel(1,1)));
}
...
fn draw_frame(c: &mut Component,edge: AxisSense, p: &Palette) {
    ...
    /* top/bottom */
    c.add_shape(fixunder_rectangle(&area(cedge(left,cpixel(0,1)),
    ...
    /* left/right */
    c.add_shape(fixunder_rectangle(&area(cedge(top,cpixel(0,18)),
...
fn measure(c: &mut Component,edge: AxisSense, p: &Palette) {
    ...
    for x in -10..10 {
        ...
        c.add_shape(tape_texture(tx,&cleaf(x as f32*100.,4).y_edge(edge),
                                 &cpixel(4,7).anchor(A_LEFT),&cpixel(1,1)));
    }
}
