...
pub fn testcard_leftright(g: Arc<Mutex<Global>>) {
    ...
    let cs = ClosureSource::new(0.,move |lc,leaf| {
        for j in 0..10 {
            ...
            let tx = text_texture(&format!("{}",i*10+j),&font,&Colour(199,208,213),&Colour(255,255,255));
            closure_add(lc,&pin_texture(tx,&cleaf(0.1*j as f32,1),
                        &cpixel(0,20),&cpixel(1,1).anchor(A_TOPLEFT)));
