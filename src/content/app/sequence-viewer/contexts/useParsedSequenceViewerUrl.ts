...
const useParsedSequenceViewerUrl = () => {
  const params = useUrlParams<'genomeId'>('/sequence-viewer/:genomeId');
  ...
  if (locationInUrl) {
    try {
      parsedLocation = getGenomicLocationFromString(locationInUrl);
    } catch {
...
export default useParsedSequenceViewerUrl;
