...
impl<Y,E> StepRun<Vec<Y>,E> for StepParallelRun<Y,E> where Y: Send {
    fn more(&mut self, _control: &mut StepControl) -> StepState2<Vec<Y>,E> {
            ...
            if runner.1.is_none() {
                match runner.0.more() {
                    StepState2::Tick => {
                        match out {
                            StepState2::Again => {},
                            _ => { out = StepState2::Tick; }
                        }
                    },
                    StepState2::Again => {
                        out = StepState2::Again;
                    },
                    StepState2::Block => {
                    },
        ...
        /* What do we do? */
        if done {
            let out : Vec<Y> = self.steps.lock().unwrap().iter_mut().map(|x| x.1.take().unwrap()).collect();
            return StepState2::Done(Ok(out));
        }
        out
    }
}
...
impl<X,Y: Send,E> StepParallel<X,Y,E> {
    pub fn new<A>(mut steps: Vec<A>) -> impl Step2<X,Vec<Y>,E> where A: Step2<X,Y,E> + 'static, Y: Send + 'static, E: 'static {
        let steps = steps.drain(..).map(|x| Box::new(x) as Box<dyn Step2<X,Y,E>>).collect();
        StepParallel {
...
mod test {
    ...
    pub fn test_parallel_smoke() {
        /* NOTE: also verifies that Again trumps Tick by verifying completion times */
        /* setup */
        ...
        let p = StepSequence2::new(StepParallel::new(vec![a,b]),z);
        x.add(p,&(),&cfg,"test");
        /* simulate */
        for i in 0..4 {
            x.tick(10.);
        ...
        x.tick(10.);
        assert!(out2.lock().unwrap().len() != 0);
        assert_eq!(vec![5,2],*out2.lock().unwrap());
    }
}