            ...
            if bs_num == 0 {
                if bs_val.len() == 1 {
                    ...
                    if let Some(v) = UNIX_UNESCAPES.get(&bs_val) {
                        ...
                        bs_val.clear();
                    } else if bs_val.chars().next().unwrap().is_digit(10) {
                        /* octal escape, collect more */
                        bs_num = 2;
                    } else {
...
fn escapes() {
    ...
    assert_eq!(string_unescape("\\999").err().unwrap(),"decoding \\999: invalid digit found in string");
