...
fn new_run(seq: &mut ZhooshSequence, z: &Zhoosh<TestProp,f64>, from: f64, to: f64, reqs: &[(Arc<Mutex<ZhooshRunState>>,f64)]) -> (TestProp,Arc<Mutex<ZhooshRunState>>) {
    let t = TestProp(Arc::new(Mutex::new(vec![])));
    let mut spec = ZhooshSpec::new(z,t.clone(),from,to);
    for (after,after_prop) in reqs {
        spec.add_trigger(after.clone(),*after_prop);
    }
    let run = seq.add(spec);
    (t,run)
}

fn run(seq: ZhooshSequence, steps: i32, now: &Arc<Mutex<i32>>) {
...
fn zhoosh_smoke() {
    ...
    let (t2,step2) = new_run(&mut seq,&z2,1.,10.,&[(step1,0.5)]);
    run(seq,23,&now);
...
fn delay_start(prop: f64, delay1: f64, delay2: f64) -> i32 {
    ...
    let (t2,step2) = new_run(&mut seq,&z2,1.,10.,&[(step1,prop)]);
    run(seq,23,&now);
...
fn zhoosh_multi_start() {
    ...
    let mut seq = ZhooshSequence::new();
    let (_,step1) = new_run(&mut seq,&z1,0.,10.,&[]);
    let (t2,step2) = new_run(&mut seq,&z2,1.,10.,&[]);
    let mut step3 = zhoosh_collect();
    step3.add_trigger(step1,1.);
    step3.add_trigger(step2,0.5);
    let run3 = seq.add(step3);
    let (t4,step4) = new_run(&mut seq,&z1,0.,10.,&[(run3,1.)]);
    run(seq,23,&now);
    print!("{:?}\n",t4.0.lock().unwrap());
    assert_eq!(t4.0.lock().unwrap().len(),7);
...
fn zhoosh_quadratic() {
    ...
    let mut seq = ZhooshSequence::new();
    let (t1,step1) = new_run(&mut seq,&z1,0.,1000.,&[]);
    run(seq,120,&now);
    print!("{:?}\n",t1.0);
