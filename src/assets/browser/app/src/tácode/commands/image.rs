...
use drawing::{ DrawingSpec, FCFont, FontVariety, bitmap_texture };
...
fn image(txx: &mut Vec<DrawingSpec>, dims: &Vec<f64>, data: &String) -> Vec<f64> {
    let data = base64::decode(data).ok().unwrap();
    let tx = bitmap_texture(data,cpixel(dims[0] as i32,dims[1] as i32),false);
    txx.push(tx);
...
pub struct Image(TáContext,usize,usize,usize);
            ...
            if let TáTask::MakeShapes(_,_,_,ref mut tx,_,_,_) = task {
                ...
                regs.get(self.2).as_floats(|dims| {
                    regs.get(self.3).as_string(|data| {
                        regs.set(self.1,Value::new_from_float(image(tx,dims,data)));
                    });
                });
            }
        });
...
impl Instruction for ImageI {
    fn signature(&self) -> Signature { Signature::new("image","rrr") }
    fn build(&self, args: &Vec<Argument>) -> Box<Command> {
